...
export async function sendWebAi(deps, input = {}) {
    ...
    try {
        ...
        if (uploadPaths.length) {
            ...
            const filesForUpload = repomixMode ? uploadFiles : uploadPaths.map(fileInfoFromPath);
            const upload = await attachLocalFilesLive(page, filesForUpload, {
        ...
        for (let index = 0; index < uploadPaths.length; index += 1) {
            ...
            const uploadFile = repomixMode ? uploadFiles[index] : fileInfoFromPath(uploadPath);
            const sentAttachment = await verifySentTurnAttachmentLive(page, uploadFile);
            if (repomixMode ? sentAttachment.ok !== true : !sentAttachment.ok) {
                usedFallbacks.push('sent-attachment-evidence-unavailable');
                const displayName = repomixMode ? uploadFile.basename : fileInfoFromPath(uploadPath).basename;
                attachmentWarnings.push(`sent attachment evidence unavailable after submit (${displayName}): ${sentAttachment.error}`);
            }
        }
