...
| ChatGPT web-AI resolver | beta | `web-ai/chatgpt.mjs`, `web-ai/chatgpt-composer.mjs`, `web-ai/chatgpt-model.mjs`, `web-ai/product-surfaces.mjs`; GPT-5.6 Chat exposes ChatGPT-only `--family` separately from `--model` tier, effort-only targets the current tier and fails closed on Pro, and requested Sol is vetoed by a final Pro composer state; Work mutation is available only through CLI `web-ai work send --prompt ... --power N` and MCP `web_ai_work_send`, while Chat commands/tools reject Work | `test/unit/web-ai-chatgpt-model.test.mjs`, `test/unit/web-ai-product-surfaces.test.mjs`, `test/unit/web-ai-provider-dom-contract.test.mjs`; fixture owners: `test/fixtures/provider-dom/chatgpt-gpt56-chat.html`, `test/fixtures/provider-dom/chatgpt-gpt56-work.html`, `test/fixtures/provider-dom/chatgpt-gpt56-eval.json` | `src/browser/web-ai/chatgpt.ts` — beta in cli-jaw; GPT-5.6 contract parity must be re-audited before a mirror-ready claim. |
...
| Blocking-IO ingress ratchet (`scripts/blocking-io-baseline.json`) | experimental | `scripts/blocking-io-gate.mjs` | `test/unit/web-ai-blocking-io-gate.test.mjs`, `npm run gate:no-new-blocking-io` | Issue #88 G3. Caps synchronous calls (`*Sync(`, `Atomics.wait`) and CDP commands per file against a committed manifest; new files default to zero. Alias, re-export, computed-member, local value alias, and non-literal `.send()` fail closed because they cannot be counted. Grandfathers existing calls — removing them is G1's job, not this gate's. Does NOT catch new callers of an existing blocking wrapper. |
