...
| --- | --- | --- | --- |
| ChatGPT | render, session store, fixture eval, model alias contract | live send/poll/query/upload/copy | composer, send button, model, upload evidence 미확인 |
...
| Answer artifact | provider poll output can be normalized through `web-ai/answer-artifact.mjs` |
| Source audit | research text can be checked through `web-ai/source-audit.mjs` before live enforcement |
...
| Known tools | `web_ai_snapshot`, `web_ai_click_ref`, `web_ai_submit_prompt`, `web_ai_wait_response`, `web_ai_copy_markdown`, `web_ai_doctor`, `web_ai_session_resume` |
...
| Benchmark | autonomous agent score가 아니라 tool substrate + external planner trajectory로 기록한다 |
