...
남긴 후속에서 WP11이 열렸다. 네 work-phase가 연쇄로 이어졌고 **각각 실제 결함을
잡았다.**
...
**회귀 테스트를 쓰려다 프로덕션 결함을 넷 잡았다.** WP8은 "테스트가 없다"는 항목이었지
"결함이 있다"는 항목이 아니었다.
...
통합 스위트가 193 → 221, 유닛이 1683 → 1690으로 늘었다. 라운드 중 추가한 회귀가
35건이다. 위 블록은 WP10 시점 값이고 WP11 이후 최종 수치는 **integration 221 /
unit 1690**이다.
