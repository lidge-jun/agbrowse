...
| web-ai 폴링 데드라인 계약 (#88) | `_plan/260731_webai_poll_deadline/` | 🔧 이 유닛 몫의 fail-open(B03/B06)과 자매 유닛이 넘긴 read 경계(B01/B02/B07/B04)를 fail-closed로 교정 + 유입 방지 게이트(`6742949`~`b3f7612`). `021`의 fail-open 여섯(B03·B06·B23·B24·B25·B36) 중 B23은 아직 관측만이다. hard deadline과 late-side-effect fencing(`93f21f0`~`6a5a2b2`) — **데드라인이 armed된 뒤** poll loop와 recovery의 async 정체가 그 안에서 반환된다(2s 예산 → 2044ms). pre-poll 구간은 범위 밖. WP21이 store lock의 blocking wait를 데드라인 경로에서 제거(`d2f442d`~`5d849f0`) — 경합 시 동기 락은 6476ms 동안 타이머를 멈추지만 async 형태는 53ms에 발화한다. 미해결 — single-flight(패배한 evaluate가 선형 누적), G4 미측정, G1 부분(나머지 동기 소비자 전환 남음), G3 partial. |
| web-ai 아티팩트·finalizer 하드닝 (#88 자매) | `_plan/260731_webai_artifact_finalizer/` | 🔧 WP11 fail-open 교정 완료 — B24/B36이 fail-closed(`2cfb668`~`25f6985`). WP19가 B25 요구 계약 확정(`020`), WP20이 구현(`4f518f7`~`41f68c8`) — `--require-file-artifacts`가 명시 요구를 fail-closed로 만들고 opportunistic은 best-effort로 남는다. B23은 관측만. 예산 계약 WP1~WP7은 미착수. |
