...
- **F15**는 도달 가능한 절반을 실제 파일시스템으로 검사한다 — staging 삭제가
  실패했을 때 link이 되돌려지는지.
- **R1·R2**(`web-ai-artifact-rollback-failure.test.mjs`)는 `node:fs`에 실패를
  주입해 나머지 절반을 덮는다. R1은 orphan 이름이 `rollbackFailed`에 담기는지,
  R2는 strict 결과의 첫 error가 `rollback-failed`인지 본다. mock을 별도 파일에
  둔 이유는 실제 artifact를 쓰는 다른 스위트로 새지 않게 하기 위해서다.

R2도 한 번 고쳤다. 처음에는 모든 `rmSync`를 실패시켰는데, 그러면 소비자 분기를
지워도 뒤의 staging sweep이 똑같이 생긴 오류를 만들어 **mutation에서 통과했다.**
처음 두 호출만 실패시키도록 바꾸고서야 그 분기를 격리한다.
