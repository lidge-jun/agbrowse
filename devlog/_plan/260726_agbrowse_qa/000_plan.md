...
### 5.1 WP6에서 하나로 묶을 것 — 실패 계약

Q7·Q11·Q12는 따로 고칠 일이 아니다. 전부 **실패를 호출자에게 어떻게 전달하는가**의
문제이고, 세 가지가 서로 다른 방식으로 어긋나 있다.
...
- **Q7, Q12**는 출력 형식 변경이다. 출력 층에서 끝난다.
- **Q11**은 종료 코드 변경이라 **셸 의미가 바뀐다.** 지금 `fetch`가 0을 낸다는
  전제로 동작하던 호출자는 이 수정 후 실패로 읽는다. 묶음 안에서 유일하게 기존
  사용자를 깨뜨릴 수 있는 항목이다.
