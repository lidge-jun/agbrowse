...
단 errorCode 카탈로그는 본문의 "14개+"가 아니라 **현재 37개** (`errorCode:` 키 리터럴 36개 + `this.errorCode =` 직접 대입 1개 = `tab-lease-store.mjs`의 `provider.active-capacity`). 주의: `grep -roh "errorCode: '...'" web-ai/ | sort -u`는 키 형태만 세어 36으로 나오므로 직접 대입 1건을 더해야 정확하다.
