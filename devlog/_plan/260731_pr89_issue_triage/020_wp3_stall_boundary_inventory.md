...
선행: WP1. WP2와는 서로 의존하지 않는다. 후행: 이 문서가 확정한 후속 유닛들.
...
1. `pollWebAi`(`web-ai/chatgpt.mjs:582`) 본문의 **모든 call expression**을
   ...
   `sync-IO/lock`을 넣으면 폐쇄가 로깅·파일 유틸까지 번질 수 있다. 발산을 막되
   구멍을 만들지 않으려면 두 가지를 **분리해서** 판정한다.

   - **횟수 bounded**: 반복 자체에 한도가 있다.
   - **시간 bounded**: 각 호출의 wall time에 상한이 있다.

   둘 다여야 bounded로 판정하고 재귀를 끊는다. `withStoreLock`
   (`web-ai/session-store.mjs:136`)이 반례다 — `LOCK_RETRY_LIMIT = 200`회로
   **횟수만** bounded이고(`:48`, `:164`), 각 반복의 `openSync`/`writeFileSync`와
   임의 callback `fn()`에는 상한이 없다. callback은 다시 동기 store 쓰기로
   이어진다(`:123-129`, `:337-344`). 따라서 backoff 반복만 bounded로 적고,
   callback과 native fs 호출은 계속 경계로 기록·배정한다.
...
`:1004-1020`(최종 타임아웃), 그리고 주 성공 반환(`:811-823`, warnings 초기화는 `:729-730`).
...
> **개정 (2026-07-31, 감사 3라운드 후).** 아래 전수성 조건은 두 번의 폐쇄 시도가
> 모두 미달하면서 **지속 가능한 완료 조건으로 부적합**하다고 판명됐다. 현재
> 스냅샷을 이론상 닫는 것은 가능하나, 비싸고 변경 즉시 낡는다. 호출 그래프가 `skills/browser/`의 탭
> 관리·CDP 연결·HTTP 폴링까지 뻗고, `deps.*`가 주입 지점이라 스냅샷이 즉시
> 낡는다. 자세한 진단은 `021_stall_boundary_map.md` §0.
>
> **새 완료 조건**은 전수가 아니라 계약이다:
>
> 1. 확인된 경계 표본과 그 처분(어느 접근으로 덮이는지)
> 2. 예산 계약 명세 — sync isolation · late-side-effect fencing · single-flight,
>    또는 격리 모델(worker/subprocess watchdog)
> 3. fail-open 목록과 교정 기준 — 예산과 독립된 별개 결함
> 4. 계약 검증 시나리오 C1~C5
> 5. 각 표본 경계의 work-phase residence
>
> 아래 전수성 조건은 **왜 그 방식이 실패했는지의 기록**으로 남긴다. 새 산출물의
> 판정 기준으로 쓰지 않는다.

**전수성 조건(완료 조건으로 부적합 — 기록용).** 세 번의 실패가 모두 "목록이 완전하다고
믿었는데 아니었다"였으므로, 완전성을 주장이 아니라 검사로 만들려 했다.
...
함수는 탭 수명주기 전체로 뻗어 있어서 재귀가 `web-ai/` 밖까지 번진다.

**그 경우에도 범위를 좁혀 완료하지 않는다** — 이것이 원래 규칙이었고, 실제로
폐쇄는 두 번 다 실패했다.

결론은 "더 열거하라"가 아니라 "열거로는 안 된다"였다. 범위를 좁힌 게 아니라
**방어 방식을 바꿨다**: 경계를 다 세는 대신 진입점 예산 계약으로 상한을 보장한다.
총량이 줄지 않는다 — 오히려 세지 못한 경계까지 덮는다는 것이 이 전환의 논거다.
`021` §0이 그 판단의 근거와 반증 조건을 담고 있다.

이 인벤토리가 경계를 20개 이상 찾아낸다면, 문제는 `pollWebAi`가 아니라 web-ai
전반의 페이지 접근 규약이다. 그때는 유닛을 늘리되 총량은 유지한다.
