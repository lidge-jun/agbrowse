...
### 로컬 프로필을 확인했다 (2026-08-01)

"로컬에 로그인이 되어 있을 것"이라는 가정으로 실제 프로필을 열어봤다.
**로그아웃 상태였다.**

```
~/.browser-agent/browser-profile/Default/Cookies   존재 (08-01 05:25)
chatgpt.com 방문 결과:
  composer          없음
  대화 sidebar      없음
  "Log in" 버튼     5개
  /api/auth/session status 200, hasUser: false
```

쿠키 파일이 있고 최근 갱신됐지만 세션은 살아 있지 않다. 파일 존재만으로
로그인을 판정하면 안 된다는 것이 여기서도 확인됐다 — `hasUser`가 실제 근거다.

따라서 조건 6은 **이 환경에서 측정 불가로 확정**한다. 추정이 아니라 확인된
사실이다.
